...
1.Įvykdykite komandą ggplot(data = mpg). Ką matote? Kodėl?
Atsiranda langas, skirtas grafikui, tačiau reikia nurodyti x, y reikšmes.

```{r}
ggplot(data = mpg)

```
Kiek eilučių lentelėje mpg? Kiek stulpelių?
Eilučių - 234, stulpelių - 11
...
```
3.Ką parodo kintamasis drv? Perskaitykite pagalbos puslapyje: ?mpg
...
```
4. Nubrėžkite sklaidos diagramą (scatterplot) pagal hwy vs. cyl.

```{r}
ggplot(mpg, aes(x = hwy, y = cyl)) + geom_point()
```

5. Nubrėšite sklaidos diagramą class vs. drv? Kodėl ji nenaudinga?
